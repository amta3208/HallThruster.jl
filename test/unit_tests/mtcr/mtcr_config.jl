...
# Import the MTCR configuration functions
include("../../../src/mtcr/mtcr_config.jl")
...
@testset "MTCR Configuration Tests" begin
    @testset "TemperatureConfig" begin
        @testset "Valid Construction" begin
            ...
            temp_config = TemperatureConfig(300.0, 300.0, 300.0, 10000.0)
        ...
        @testset "Invalid Construction" begin
            ...
            @test_throws ErrorException TemperatureConfig(-100.0, 300.0, 400.0, 10000.0)
            @test_throws ErrorException TemperatureConfig(300.0, -100.0, 400.0, 10000.0)
            @test_throws ErrorException TemperatureConfig(300.0, 300.0, -400.0, 10000.0)
            @test_throws ErrorException TemperatureConfig(300.0, 300.0, 400.0, -10000.0)

            # Test zero temperatures
            @test_throws ErrorException TemperatureConfig(0.0, 300.0, 400.0, 10000.0)
            @test_throws ErrorException TemperatureConfig(300.0, 0.0, 400.0, 10000.0)
            @test_throws ErrorException TemperatureConfig(300.0, 300.0, 0.0, 10000.0)
            @test_throws ErrorException TemperatureConfig(300.0, 300.0, 400.0, 0.0)
        end
    end
    ...
    @testset "TimeIntegrationConfig" begin
        @testset "Valid Construction" begin
            # Test basic construction
            time_config = TimeIntegrationConfig(1e-6, 1e-4, 1e-3, 1000, 2)
            ...
            # Test with defaults
            time_config2 = TimeIntegrationConfig(1e-6, 1e-4, 1e-3)
        ...
        @testset "Invalid Construction" begin
            ...
            @test_throws ErrorException TimeIntegrationConfig(-1e-6, 1e-4, 1e-3)
            @test_throws ErrorException TimeIntegrationConfig(1e-6, -1e-4, 1e-3)
            @test_throws ErrorException TimeIntegrationConfig(1e-6, 1e-4, -1e-3)

            # Test zero time parameters
            @test_throws ErrorException TimeIntegrationConfig(0.0, 1e-4, 1e-3)
            @test_throws ErrorException TimeIntegrationConfig(1e-6, 0.0, 1e-3)
            @test_throws ErrorException TimeIntegrationConfig(1e-6, 1e-4, 0.0)

            # Test invalid nstep
            @test_throws ErrorException TimeIntegrationConfig(1e-6, 1e-4, 1e-3, 0)
            @test_throws ErrorException TimeIntegrationConfig(1e-6, 1e-4, 1e-3, -100)

            # Test invalid method
            @test_throws ErrorException TimeIntegrationConfig(1e-6, 1e-4, 1e-3, 1000, 3)
            @test_throws ErrorException TimeIntegrationConfig(1e-6, 1e-4, 1e-3, 1000, -1)
        end
    end
    ...
    @testset "MTCRConfig" begin
        ...
        @testset "Custom Construction with All Parameters" begin
            ...
            temperatures = TemperatureConfig(300.0, 300.0, 300.0, 50000.0)
            time_params = TimeIntegrationConfig(1e-6, 1e-4, 1e-2)
            physics = PhysicsConfig(bbh_model = 2)
            processes = ProcessConfig(consider_rad = 1)
            ...
            @test config.library_path == temp_mtcr_path
    ...
    @testset "validate_config" begin
        @testset "Valid Inputs" begin
            ...
            total_number_density = 1e13
            temperatures = TemperatureConfig(300.0, 300.0, 300.0, 10000.0)
            time_params = TimeIntegrationConfig(1e-6, 1e-4, 1e-3)
            library_path = ""
            case_path = pwd()
            unit_system = :CGS

            @test validate_config(species, mole_fractions, total_number_density,
    ...
    @testset "generate_input_files" begin
        @testset "Directory Structure Creation" begin
            ...
            try
                ...
                @test generate_input_files(config, temp_dir) == true
        ...
        @testset "File Content Validation" begin
            ...
            try
                config = nitrogen_10ev_config()
                generate_input_files(config, temp_dir)
    ...
    @testset "get_molecular_weights" begin
        @testset "Known Species" begin
            ...
            weights = get_molecular_weights(["N", "N2", "N+", "N2+", "E-"])
            ...
            weights_ar = get_molecular_weights(["Ar", "Ar+"])
            @test weights_ar[1] ≈ 39.948  # Ar
            @test weights_ar[2] ≈ 39.948  # Ar+

            weights_xe = get_molecular_weights(["Xe", "Xe+"])
            @test weights_xe[1] ≈ 131.293  # Xe
            @test weights_xe[2] ≈ 131.293  # Xe+
        end

        @testset "Unknown Species" begin
            @test_throws ErrorException get_molecular_weights(["UNKNOWN"])
            @test_throws ErrorException get_molecular_weights(["N", "UNKNOWN", "E-"])
        end
    end

    @testset "validate_species_against_mtcr_database" begin
        @testset "Warning for Unimplemented Feature" begin
            config = MTCRConfig(
                species = ["N", "N2", "E-"],
                mole_fractions = [0.1, 0.8, 0.1],
                total_number_density = 1e13,
            ...
            # Should return true but issue a warning
            @test validate_species_against_mtcr_database(config) == true
        end

        @testset "Skip Validation When Disabled" begin
            config = MTCRConfig(
                species = ["N", "N2", "E-"],
                mole_fractions = [0.1, 0.8, 0.1],
                total_number_density = 1e13,
    ...
    @testset "convert_config_units" begin
        @testset "No Conversion Needed" begin
            config = nitrogen_10ev_config()  # Default is CGS
            converted = convert_config_units(config, :CGS)
            @test converted === config  # Should return same object
        end

        @testset "SI to CGS Conversion" begin
            # Create SI config
            config_si = MTCRConfig(
                species = ["N", "N2", "E-"],
                mole_fractions = [0.1, 0.8, 0.1],
            ...
            converted = convert_config_units(config_si, :CGS)
        ...
        @testset "CGS to SI Conversion" begin
            config_cgs = nitrogen_10ev_config()  # CGS by default
            converted = convert_config_units(config_cgs, :SI)
        ...
        @testset "Invalid Conversion" begin
            config = nitrogen_10ev_config()
            @test_throws ErrorException convert_config_units(config, :INVALID)
        end
    end

    @testset "create_propellant_config" begin
        @testset "Nitrogen Configuration" begin
            config = create_propellant_config(:nitrogen)
            @test config.species == ["N", "N2", "N+", "N2+", "E-"]
            @test config.mole_fractions == [1.0e-20, 0.9998, 1.0e-20, 0.0001, 0.0001]
            @test occursin("n2", config.database_path)
            @test config.temperatures.Te == 115000.0  # Default electron temperature
            @test config.temperatures.Tt == 300.0     # Default gas temperature
        end

        @testset "Argon Configuration" begin
            config = create_propellant_config(:argon)
            @test config.species == ["Ar", "Ar+", "E-"]
            @test config.mole_fractions == [0.9998, 0.0001, 0.0001]
            @test occursin("argon", config.database_path)
        end

        @testset "Xenon Configuration" begin
            config = create_propellant_config(:xenon)
            @test config.species == ["Xe", "Xe+", "E-"]
            @test config.mole_fractions == [0.9998, 0.0001, 0.0001]
            @test occursin("xenon", config.database_path)
        end

        @testset "Krypton Configuration" begin
            config = create_propellant_config(:krypton)
            @test config.species == ["Kr", "Kr+", "E-"]
            @test config.mole_fractions == [0.9998, 0.0001, 0.0001]
            @test occursin("krypton", config.database_path)
        end
        ...
        @testset "Custom Parameters" begin
            ...
            @test config.temperatures.Te == 50000.0
            @test config.temperatures.Tt == 500.0
            @test config.total_number_density == 1e12
        end

        @testset "Invalid Propellant" begin
            @test_throws ErrorException create_propellant_config(:helium)
            @test_throws ErrorException create_propellant_config(:invalid)
        end
    end
