...
# Import the MTCR data conversion functions
...
include("../../../src/mtcr/data_conversion.jl")
...
@testset "Unit Conversions" begin
    @testset "Density Conversions" begin
        ...
        rho_cgs = convert_density_si_to_cgs(rho_si)
        @test rho_cgs ≈ rho_cgs_expected

        # Test CGS to SI density conversion
        rho_si_back = convert_density_cgs_to_si(rho_cgs)
        @test rho_si_back ≈ rho_si

        # Test with zero values
        @test convert_density_si_to_cgs([0.0]) ≈ [0.0]
        @test convert_density_cgs_to_si([0.0]) ≈ [0.0]

        # Test with very small values
        small_si = [1e-10]
        small_cgs = convert_density_si_to_cgs(small_si)
        @test convert_density_cgs_to_si(small_cgs) ≈ small_si

        # Test with very large values
        large_si = [1e10]
        large_cgs = convert_density_si_to_cgs(large_si)
        @test convert_density_cgs_to_si(large_cgs) ≈ large_si
    end
    ...
    @testset "Energy Density Conversions" begin
        ...
        energy_cgs = convert_energy_density_si_to_cgs(energy_si)
        @test energy_cgs ≈ energy_cgs_expected

        # Test CGS to SI energy density conversion
        energy_si_back = convert_energy_density_cgs_to_si(energy_cgs)
        @test energy_si_back ≈ energy_si

        # Test with zero
        @test convert_energy_density_si_to_cgs(0.0) ≈ 0.0
        @test convert_energy_density_cgs_to_si(0.0) ≈ 0.0

        # Test roundtrip with various values
        test_values = [1.0, 1e3, 1e6, 1e9, 1e-6]
        for val in test_values
            converted = convert_energy_density_si_to_cgs(val)
            back = convert_energy_density_cgs_to_si(converted)
    ...
    @testset "Pressure Conversions" begin
        ...
        pressure_cgs = convert_pressure_si_to_cgs(pressure_si)
        @test pressure_cgs ≈ pressure_cgs_expected

        # Test CGS to SI pressure conversion
        pressure_si_back = convert_pressure_cgs_to_si(pressure_cgs)
        @test pressure_si_back ≈ pressure_si

        # Test with typical Hall thruster pressures
        ht_pressure_si = 1e-2  # Pa (low pressure)
        ht_pressure_cgs = convert_pressure_si_to_cgs(ht_pressure_si)
        @test convert_pressure_cgs_to_si(ht_pressure_cgs) ≈ ht_pressure_si

        # Test roundtrip consistency
        test_pressures = [1.0, 1e3, 1e5, 1e-3, 1e-6]
        for p in test_pressures
            converted = convert_pressure_si_to_cgs(p)
            back = convert_pressure_cgs_to_si(converted)
    ...
    @testset "Number Density Conversions" begin
        ...
        n_cgs = convert_number_density_si_to_cgs(n_si)
        @test n_cgs ≈ n_cgs_expected

        # Test CGS to SI number density conversion
        n_si_back = convert_number_density_cgs_to_si(n_cgs)
        @test n_si_back ≈ n_si
        ...
        for (si_val, cgs_expected) in zip(
            ...
            cgs_val = convert_number_density_si_to_cgs(si_val)
            @test cgs_val ≈ cgs_expected
            @test convert_number_density_cgs_to_si(cgs_val) ≈ si_val
        end

        # Test roundtrip with edge cases
        edge_cases = [1.0, 1e25, 1e-10]
        for n in edge_cases
            converted = convert_number_density_si_to_cgs(n)
            back = convert_number_density_cgs_to_si(converted)
...
@testset "State Vector Conversions" begin
    @testset "SI to CGS State Conversion" begin
        ...
        result = convert_state_si_to_cgs(rho_sp_si, rho_etot_si, number_density_si)
        ...
        result_full = convert_state_si_to_cgs(rho_sp_si, rho_etot_si, number_density_si;
        ...
        nitrogen_state = convert_state_si_to_cgs([n2_density_si, n_density_si],
    ...
    @testset "CGS to SI State Conversion" begin
        ...
        result = convert_state_cgs_to_si(rho_sp_cgs, rho_etot_cgs)
        ...
        # Test with optional energy components
        result_full = convert_state_cgs_to_si(rho_sp_cgs, rho_etot_cgs;
            rho_erot_cgs = 1e3,
            rho_eeex_cgs = 2e3,
            rho_evib_cgs = 3e3,)
        ...
        cgs_state = convert_state_si_to_cgs(
            original_rho_sp, original_rho_etot, original_number_density,)
        si_state = convert_state_cgs_to_si(cgs_state.rho_sp, cgs_state.rho_etot)
    ...
    @testset "Source Terms Conversion" begin
        ...
        result = convert_sources_cgs_to_si(drho_sp_cgs, drho_etot_cgs)

        @test result.drho_sp ≈ [100.0, 200.0, -50.0]  # kg/m³/s
        @test result.drho_etot ≈ 1e1  # J/m³/s

        # Test with optional source terms
        result_full = convert_sources_cgs_to_si(drho_sp_cgs, drho_etot_cgs;
            drho_erot_cgs = 10.0,
            drho_eeex_cgs = 20.0,
            drho_evib_cgs = 30.0,)
...
@testset "Data Validation and Preparation" begin
    @testset "Array Preparation for Fortran" begin
        ...
        prepared = prepare_arrays_for_fortran(julia_array, scalar_val, nothing_val)
        ...
        int_array = [1, 2, 3]
        prepared_int = prepare_arrays_for_fortran(int_array)
        @test prepared_int[1] isa Array{Float64}
        @test prepared_int[1] == [1.0, 2.0, 3.0]

        # Test with 2D arrays
        matrix = [1.0 2.0; 3.0 4.0]
        prepared_matrix = prepare_arrays_for_fortran(matrix)
        @test prepared_matrix[1] isa Array{Float64}
        @test size(prepared_matrix[1]) == (2, 2)

        # Test with empty arrays
        empty_array = Float64[]
        prepared_empty = prepare_arrays_for_fortran(empty_array)
        ...
        arr2 = [3.0, 4.0, 5.0]
        scalar = 10.0
        prepared_multi = prepare_arrays_for_fortran(arr1, arr2, scalar)
    ...
    @testset "Species Data Validation" begin
        ...
        @test validate_species_data(species_names, mtcr_species, densities) == true

        # Test mismatched array lengths
        @test_throws ErrorException validate_species_data(
        ...
        @test_logs (:warn, r"Very small densities detected") validate_species_data(
            species_names, mtcr_species, small_densities,)

        # Test with zero densities (valid)
        zero_densities = [1e-3, 0.0, 1e-7, 1e-7, 1e-7]
        @test validate_species_data(species_names, mtcr_species, zero_densities) == true

        # Test with realistic Hall thruster species
        ht_species = ["Xe", "Xe+"]
        ht_mtcr_species = ["Xe", "Xe+", "Xe2+", "E-"]
        ht_densities = [1e-4, 1e-5]
        @test validate_species_data(ht_species, ht_mtcr_species, ht_densities) == true
    end
    ...
    @testset "Species Mapping" begin
        ...
        mapping = create_species_mapping(ht_species, mtcr_species)
        ...
        noble_mapping = create_species_mapping(noble_ht, noble_mtcr)
        ...
        direct_mapping = create_species_mapping(direct_ht, direct_mtcr)
        ...
        @test_throws ErrorException create_species_mapping(missing_ht, mtcr_species)

        # Test error for unmappable species
        unmappable_ht = ["UNKNOWN_SPECIES"]
        unmappable_mtcr = ["N2", "N"]
        @test_throws ErrorException create_species_mapping(unmappable_ht, unmappable_mtcr)

        # Test E- vs e- mapping
        electron_ht = ["E-"]
        electron_mtcr = ["E-"]
        electron_mapping = create_species_mapping(electron_ht, electron_mtcr)
...
@testset "Chemistry Conversions" begin
    @testset "Mole Fractions to Mass Densities" begin
        ...
        mass_densities = mole_fractions_to_mass_densities(
        ...
        equal_densities = mole_fractions_to_mass_densities(
            equal_fractions, equal_weights, 1e15,)

        @test all(equal_densities .≈ equal_densities[1])  # Should all be equal

        # Test error conditions
        @test_throws ErrorException mole_fractions_to_mass_densities(
    ...
    @testset "Mass Densities to Mole Fractions" begin
        ...
        mole_fractions = mass_densities_to_mole_fractions(mass_densities, molecular_weights)
        ...
        fractions = mass_densities_to_mole_fractions(equal_masses, different_weights)

        # Lighter species should have higher mole fraction
        @test fractions[1] > fractions[2] > fractions[3]

        # Test error conditions
        @test_throws ErrorException mass_densities_to_mole_fractions([1e-10, 1e-10], [20.0])  # Mismatched lengths
        @test_throws ErrorException mass_densities_to_mole_fractions(
    ...
    @testset "Roundtrip Consistency" begin
        ...
        # Backward conversion
        recovered_fractions = mass_densities_to_mole_fractions(
        ...
        extreme_masses = mole_fractions_to_mass_densities(
            extreme_fractions, molecular_weights, 1e20,)
        extreme_recovered = mass_densities_to_mole_fractions(
        ...
        single_fraction = [1.0]
        single_weight = [20.0]
        single_mass = mole_fractions_to_mass_densities(single_fraction, single_weight, 1e16)
        single_recovered = mass_densities_to_mole_fractions(single_mass, single_weight)
        ...
        xe_masses = mole_fractions_to_mass_densities(xe_fractions, xe_weights, xe_density)
        xe_recovered = mass_densities_to_mole_fractions(xe_masses, xe_weights)
